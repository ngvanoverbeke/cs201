...
eesy.define([
    ...
    'json!language', 'helpitem-handling'
], function ($, engineState, utils, mouse, Mustache, language, helpitemHandling) {
    ...
    function scrollAdjust() {
        var adjustScrollbarZd = ($(".patient-bar-container__content").get(0).scrollHeight > $(".patient-bar-container__content").height());
    ...
    function launchSupportCenter(hid) {
        eesyRequire(['supportCenter'], function (supportCenter) {
            if (var_open_dashboard_in_new_window) {
                ...
                supportCenter.show(helpitemHandling.getFoundItemsString(), engineState.foundNodes.get().getFoundItemsString(), hid);
            }
        });
    }
